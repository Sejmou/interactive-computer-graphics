...
if (descriptionParagraph) {
    descriptionParagraph.innerHTML = String.raw`B-Spline curves are a generalization of Bèzier curves with a very practical property: In contrast to Bézier curves their control points only have local control.<br>
...
Unfortunately, notation across math publications is inconsistent, for example some use \(u\) instead of \(t\), sometimes also \(S\) or \(s\) are used instead of \(C\).<br>

Note that, contrary to Bézier curves, the degree and order of B-Spline curves are independent of the number of control points.<br>
...
We can think of B-Splines as piecewise polynomial functions of degree \( p \) that meet at the knots of the knot vector.<br>


The relationship \( m = n + k \) is a requirement for any B-Spline curve (otherwise it is not properly defined).<br>
If this relationship is given, this allows us to properly define the basis functions \(N_{i,p}\) of the B-Spline curve.<br>
The basis functions are computed using in a recursive way that can be visualized as a triangular scheme.<br>
...
B-Spline curves are only defined in the interval for \(t\) where the condition \(\sum_{i=0}^{n}{N_{i,p}(t) \cdot P_{i}} = 1\) is satisfied.<br>
We differentiate between open and closed B-Spline curves.<br>
